...
export function SlicerVisual({ visual, data, onFilter, activeFilters }: SlicerVisualProps) {
  ...
  const tableData = getTable(data, valuesField.table)
  if (!tableData || tableData.length === 0) {
    return <div className="text-muted-foreground text-xs p-4">No data</div>
  }

  // Resolve the column with tolerant matching (handles "Revenue Won" vs "Revenue_Won")
  let col = resolveColumn(tableData[0], valuesField.column)
  if (!col) {
    // Fallback: first string column in the table
